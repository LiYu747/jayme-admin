    ...
    submitForm(formName) {
      ...
      _self.$refs[formName].validate(async (valid) => {
        ...
        if(valid){
            ...
            let ret = null;
            if (_self.rowData) {
                ret = await _self.$http.modifyGalleryType(_self.form)
            }
            else{
                ret = await _self.$http.addGalleryType(_self.form)
            }
            console.log(ret);
            if(ret.data.code == 0){
                ElMessage({
                ...
                if (_self.rowData) {
                    _self.$http.operateLogSave({
                      ...
                      detail:`管理员 [${_self.userInfo.name}] 编辑了图集分类[${_self.form.title}]`
                    })
                }
                else{
                    _self.$http.operateLogSave({
                      operateLogType: 'ADMIN_OPT',
                      detail:`管理员 [${_self.userInfo.name}] 新增了图集分类[${_self.form.title}]`
                    })
                }
