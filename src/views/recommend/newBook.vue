    ...
    <div class="card_header">
      ...
      <div class="card_header_right">
        <el-popover
          ...
          :width="200"
          trigger="hover"
          content="注:新上书籍只有排序前六的才能在小程序首页展示"
        >
          <template #reference>
          ...
          >
            ...
            <el-table-column property="address" label="操作">
              <template #default="scope">
                ...
                >
                  {{ scope.row.hasExit == false ? "添加" : "已添加" }}</el-link
                >
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    getList() {
      ...
      _this.$http.getNewBookList(from).then((res) => {
        console.log(res);
    ...
    add(val) {
      ...
      this.$http.addNewBook(from).then((res) => {
        // console.log(res);
        if (res.data.code == 0) {
          val.hasExit = true;
          ...
          this.$http
            .operateLogSave({
              ...
              detail:
                "管理员" + this.userInfo.name + "添加了新上书籍" + val.name,
            })
            .then(() => {
              // this.getList();
            });
        ...
        if (res.data.code == 0) {
          ...
          this.$http
            .operateLogSave({
              operateLogType: "ADMIN_OPT",
              detail:
                "管理员" + this.userInfo.name + "批量添加了新上书籍" + tempText,
            })
            .then(() => {
              this.getAddResList();
            });
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$http.updateNewBook(from).then((res) => {
            console.log(res);
            if (res.data.code == 0) {
              ElMessage({
                message: res.data.msg,
    ...
    del(val) {
      ...
      this.$confirm("是否取消推荐?", "提示", {
        ...
        this.$http.deleteNewBook(from).then((res) => {
          if (res.data.code == 0) {
            ElMessage({
              message: res.data.msg,
              type: "success",
